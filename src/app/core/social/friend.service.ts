...
export class FriendService {
  ...
  public initHub(userId: string) {
    ...
    if (!this.handlersRegistered) {
      ...
      this.hubConnection.on('VisitInviteAccepted', (receiverUsername: string) => {
        this.visitAcceptedEvents.next(receiverUsername);
      });

      this.hubConnection.on('PetPositionSynced', (userId: string, x: number, y: number) => {
        this.roomMoveEvents.next({ userId, x, y });
      });

      this.hubConnection.on('RoomMessageReceived', (userId: string, username: string, message: string) => {
      ...
      this.hubConnection.on('RoomDecorSynced', (userId: string, instances: DecorInstance[]) => {
  ...
  private async ensureConnected(): Promise<boolean> {
    ...
    if (this.getHubState() === signalR.HubConnectionState.Disconnected) {
      if (!this.connectPromise) {
        this.connectPromise = this.hubConnection.start()
          .then(() => {
            console.log('SignalR connected');
            if (this.currentUserId) {
              return this.hubConnection.invoke('JoinUserGroup', this.currentUserId);
            }
  ...
  public sendVisitInvite(senderId: number, receiverId: number): Observable<any> {
    return this.http.post(`${this.apiUrl}/visit-invite?senderId=${senderId}&receiverId=${receiverId}`, {});
  }

  public respondToVisitInvite(inviteId: number, accept: boolean): Observable<any> {
    return this.http.post(`${this.apiUrl}/visit-respond?inviteId=${inviteId}&accept=${accept}`, {});
  }

  public async joinRoom(roomOwnerId: string, userId: string) {
    if (!(await this.ensureConnected())) return;
    if (this.hubConnection?.state === signalR.HubConnectionState.Connected) {
      this.hubConnection.invoke('JoinRoom', roomOwnerId, userId)
  ...
  public async syncPetPosition(roomOwnerId: string, x: number, y: number, userId: string) {
    if (!(await this.ensureConnected())) return;
    if (this.hubConnection?.state === signalR.HubConnectionState.Connected) {
      this.hubConnection.invoke('SyncPetPosition', roomOwnerId, x, y, userId)
        .catch(err => console.error('SyncPetPosition failed:', err));
    }
  }

  public async sendRoomMessage(roomOwnerId: string, message: string, userId: string, username: string) {
    if (!(await this.ensureConnected())) return;
    if (this.hubConnection?.state === signalR.HubConnectionState.Connected) {
      this.hubConnection.invoke('SendRoomMessage', roomOwnerId, message, userId, username)
  ...
  public async kickUser(roomOwnerId: string, userIdToKick: string) {
    if (!(await this.ensureConnected())) return;
    if (this.hubConnection?.state === signalR.HubConnectionState.Connected) {
      this.hubConnection.invoke('KickUser', roomOwnerId, userIdToKick)
        .catch(err => console.error('KickUser failed:', err));
    }
  }

  public async syncRoomDecor(roomOwnerId: string, userId: string, instances: DecorInstance[]) {
    if (!(await this.ensureConnected())) return;
    if (this.hubConnection?.state === signalR.HubConnectionState.Connected) {
      this.hubConnection.invoke('SyncRoomDecor', roomOwnerId, userId, instances)
  ...
  private refreshVisitInvitations() {
    const userId = localStorage.getItem('userId');
    if (!userId) return;
    this.http.get<VisitInvite[]>(`${this.apiUrl}/visit-invites?userId=${userId}`).subscribe(invites => {
